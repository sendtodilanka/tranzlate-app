...
> **⚠ Superseded IA (2026-07-24, issue #26 — D-5 rev.2):** this plan's Compact information architecture — **canvas quick-action tiles (Conversation · Camera) instead of a bottom nav bar** — is **reversed**. The app now uses a **persistent bottom `NavigationBar` (Home / Chat / Camera)** via `NavigationSuiteScaffold`; the quick-action tiles are removed and the drawer is secondary-only.
>
> **⚠ Superseded AGAIN (2026-07-26, issue #42 / PR #43 — D-5 rev.3):** the bottom nav of rev.2 is **also** gone, and so is the drawer. Home is now the approved Claude Design card stack (UI_SPEC §2.1) — **no bar, no drawer, no FAB**. Concretely, from the plan below: **§2's `TranzlateTopBar` / `ModeChip` / `QuickActionTile` / `DottedRingIconButton` are dead**, and `ComposerCard` is superseded by the design's input card (the language chips moved out of it into a pinned row). **What still stands:** the Stage-A docs-truth work (C-2, palette), `ResultBlock` / `ShimmerResult` / `InlineErrorRetry` / `EngineBadge`, the `TextUiState` + `TextViewModel` shape, the golden-table test approach, and the Home→Result container transform. The drawer + push/scale motion work in §3 is **retired code**. See DECISIONS **D-5 rev.3**.
