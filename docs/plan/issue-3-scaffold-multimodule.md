...
- **Adaptive nav** — `NavigationSuiteScaffold` bar (Compact) → rail (Medium) → permanent drawer (Expanded, `layoutType` override). **Primary destinations (D-5 rev.2 — issue #26):** Home (Text) · Chat (v2 placeholder) · Camera. History සහ Settings දැන් **drawer secondary** — bottom bar එකේ නෑ (toggle-aware — §4 R2). `NavigationSuiteScaffold` tooling තේරීම D-5 ට කලින්මයි, අලුත් දිශාවට එලෙසම ගැලපෙනවා.
