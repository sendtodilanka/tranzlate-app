...
> **⚠ Superseded nav-shell note (2026-07-26, issue #42 / PR #43 → DECISIONS D-5 rev.3):** every "adaptive nav shell (`NavigationSuiteScaffold`)" claim in this plan — the title, the §Scope line, the `material3-adaptive-navigation-suite` dependency row (§1), the `:app` description (§65) and the **Adaptive nav** bullet (§73) — is **superseded**. The approved design has **no navigation component at all**: `:app`'s shell is a bare Nav3 `NavDisplay` and Home's card stack carries the IA. The `material3-adaptive-navigation-suite` dependency is now unused. **Everything else in this plan stands** — module graph, version catalog, build-logic convention plugins, Hilt wiring, flavors, the fake-engine variant, CI, and the R2 toggle-aware nav-registry rule (which still applies: the camera entry lives in `:app`, filtered by `AppConfig.featureToggles`).
...
- **Adaptive nav** — ⚠ **SUPERSEDED 2026-07-26 (D-5 rev.3):** navigation component එකක් **නෑ** — shell එක bare `NavDisplay` එකක්, Home එකේ card stack එකම තමයි IA එක. `NavigationSuiteScaffold` dependency එක දැන් unused. Toggle-aware nav-registry rule එක (§4 R2) එහෙමම තියෙනවා. ~~`NavigationSuiteScaffold` bar (Compact) → rail (Medium) → permanent drawer (Expanded, `layoutType` override). **Primary destinations (D-5 rev.2 — issue #26):** Home (Text) · Chat (v2 placeholder) · Camera. History සහ Settings **drawer secondary**.~~
