...
> ⚠️ **AMENDED after this ruling was accepted** (2026-08-01, issue #180 · evidence: the rev 5 export itself).
> **19j is cut.** Decoding the spec's single bundler block gives 54 `data-screen-label` frames (52 unique — **two unrelated screens, 16a and 20b, both label their frames with the bare `light`/`dark` string** instead of a qualified one like `19a · light`; the first version of this note called it "the 16a pair", which is wrong). Verified by a method that does not depend on those labels at all: `<div class="dv-opt" id="19i">` and `id="19j"` both exist and contain **only** a "Cut in rev 5" caption and an anchor — no device frame, no `LIGHT`/`DARK` markup. **There is no drawing, as distinct from a drawing missing its label.** The document states the reason where 19j stood: *"'No offline voice' was opened by tapping the speaker mark — but the mark is only drawn where an offline voice exists."*
> **The §7.6 dead-marks REJECT still holds — rev 5 satisfies it by deletion instead of construction.** A mark on every row does nothing on rows without a voice, which is the dead affordance §7.6 refused; drawing the mark **only where the voice exists** removes the dead case, so there is nothing left to explain and the sheet has no trigger.
> **PR-12's scope therefore becomes:** speaker mark **conditional on the device's installed offline voice** · voice legend · "Recently used as target" (per-role recents; empty → section absent) · `Selected(inner: LanguageRowState)`. **No 19j sheet.** Tests: row voice-flag matrix, target-recent ordering — **`19j trigger` is dropped**, nothing can fire it. Previews: `TargetPickerRow{Voice,NoVoice,Selected}Preview`, `VoiceLegendPreview` — **`NoOfflineVoiceSheetPreview` is dropped**.
...
| R4 | Voice marks silently empty / false positive | **E-V1**: AOSP-no-GTTS image → empty set + marks absent; *(**amended 2026-08-01, #180 · corrected by the #181 co-verify lens** — this row's acceptance also read "19j copy coherent". **That half was never run.** PR-10 (#147) explicitly deferred it for want of UI — *"Deferred: 'no marks appear, 19j copy coherent' needs UI, which is PR-12"*, and `docs/research/issue-130-e-v1-voice-enumeration.md:126-129` says the same. Only the enumeration half — empty set on a no-engine device — was pinned. Now that 19j is cut, the deferred half has nothing left to check against: the clause is **void, not discharged**. The first version of this amendment said it "was run and accepted" and "the result stands", which would have told a future R4 audit the criterion had been met.)* `<queries>` නැතුව API 36 + Google TTS → empty ලැබෙන බව පෙන්වා block එකේ අවශ්‍යතාවය pin |
...
6. **P1 16a-with-dead-marks sequencing** — 19j = mark tap target drawn (spec 16a/19j); PR-12 එකේම දෙකම. **(AMENDED 2026-08-01 · #180: rev 5 එකේ 19j කපා ඇත. Mark එක offline voice එකක් ඇති row වලට පමණක් අඳින නිසා dead mark එකක් නෑ — §7.6 REJECT එක තවමත් valid, ඒත් එය සපුරන්නේ sheet එකක් හදලා නොව, dead case එකම ඉවත් කිරීමෙන්. PR-12 එකේ 19j නෑ.)**
