...
> **19j is cut.** Decoding the spec's single bundler block gives 54 `data-screen-label` frames (52 unique — `light`/`dark` are the 16a pair), and **no 19j and no 19i frame exists**. The document states the reason where 19j stood: *"'No offline voice' was opened by tapping the speaker mark — but the mark is only drawn where an offline voice exists."*
...
| R4 | Voice marks silently empty / false positive | **E-V1**: AOSP-no-GTTS image → empty set + marks absent; *(**amended 2026-08-01, #180** — this row's acceptance also read "19j copy coherent". E-V1 was run and accepted in PR-10 (#147) against copy for a sheet rev 5 had cut. The experiment's result stands; the clause is void.)* `<queries>` නැතුව API 36 + Google TTS → empty ලැබෙන බව පෙන්වා block එකේ අවශ්‍යතාවය pin |
