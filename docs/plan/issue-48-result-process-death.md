...
- Two new `SavedStateHandle` keys hold what cannot be recomputed: the translated text and the engine. The request is already persisted for Retry, and `transliteration` is always null.
...
Unit (`:feature:text`, 13 tests, all green — 3 new):
...
| **after process death** | **result ✗, actions ✗** | **result ✅, actions ✅** |
