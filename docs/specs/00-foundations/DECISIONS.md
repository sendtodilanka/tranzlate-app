...
| D-4 | Interstitial ad policy (free tier) | **Revenue-optimized, AdMob-compliant:** after every **N=2** completed translations, min gap **90s**, daily cap **12**. All three RemoteConfig-tunable (`ad_nth`, `ad_min_gap_s`, `ad_daily_cap`). Never on Back-press / utility navigation / task start. **Suppressed entirely for PRO (entitlement resolves to `Paid(PRO)`) — issue #50.** |
...
| C-3 | string-key authority (**enforced** 2026-08-01, issue #152) | **`STRINGS_*.md` is the ONLY key authority.** contentDescriptions use its `cd_*` keys. TEST_A11Y references those keys — never invents `a11y_*`/`cd_*` of its own. Missing keys get ADDED to STRINGS. **Now a build gate:** the `verifyStringKeyDocs` Gradle task fails `check` (and CI) when a shipped `<string>`/`<plurals>` key appears in no `STRINGS_*.md`. It was a rule with no check for a year of PRs, and PR-6 broke it in the ordinary way — `cd_lang_back` into three locale files and into no catalogue, every gate green, caught only because a co-verify lens happened to read the PR body. The gate runs **resource → catalogue only**; the reverse is deliberately not enforced, because catalogues here are written *before* the resource ships and deliberately keep retired keys on record. A key struck through in a catalogue does **not** count as documented. Catalogues today: `STRINGS_text-translation.md`, `STRINGS_settings.md`, `STRINGS_language.md`, `STRINGS_paywall.md`, `STRINGS_history.md`, `STRINGS_shell.md`. |
| C-4 **rev.2** (2026-08-02, issue #174) | live-region strings | Canonical keys **in STRINGS**, each with its mode: `a11y_translating`="Translating…" → **`Polite`**, host `tt_text_loading`; `a11y_result_ready`="Translation ready: %1$s" → **`Polite`**, host `tt_text_result`; `a11y_error`="Translation failed. %1$s" → **`Assertive`**, host `tt_text_error_view`. Assertive is for outcomes that END the task; a progress notice never interrupts (it fires while TalkBack is still reading the control's own label). ~~`a11y_limit_reached`="Daily Advanced-AI limit reached"~~ — **struck by #174**, and it shipped in no locale for the life of the project. Three reasons: (1) `TextUiState.Limit` carries two different truths — quota spent vs entitlement denied — and one fixed string is FALSE for the second, telling a screen-reader user to wait for a midnight reset that will grant them nothing; (2) the state is already announced per-kind and assertively — `tt_text_limit` reads `text_error_limit_reached`/`text_error_not_entitled` in both the portrait face and the pane, and the C-11 sheet reads `limit_sheet_title_quota`/`limit_sheet_title_pro`; (3) the key never agreed with itself — C-4 said "Daily Advanced-AI limit reached", TEST_A11Y §2.3 said "Daily free limit reached", the shipped sheet says "Daily free AI limit reached". Three values, one key, no resource. *(rev.1 named four keys and pinned bare `en` values that had already drifted from the shipped copy.)* |
