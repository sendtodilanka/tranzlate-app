...
| `text_lang_library_free` | plurals | `of %1$d packs · %2$s free` | offline-capable, size label | ⚠ the **degrade** line (risk R8). The store directory is absent, renamed or empty while packs are installed, so the size is unknown — free space is reported instead. `0 MB used` would be a claim about the disk that nothing checked |
| `text_lang_library_none` | plurals | `of %1$d packs · nothing downloaded` | offline-capable | outranks both of the above: a device with no packs has no store to walk either, so a null byte answer there is ordinary rather than a fault. Verified on `emulator-5554` — the store dir did not exist until the first pack landed (E-S1, `docs/research/issue-130-e-s1-storage-walk.md`) |
