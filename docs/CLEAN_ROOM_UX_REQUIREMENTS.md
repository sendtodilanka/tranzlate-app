...
3. Navigation reflects information architecture: the 3-5 real peer tasks (Text, Camera, Dialog, History) live as sibling top-level destinations in a single NavigationSuiteScaffold, one tap from each other with saved per-tab state; the drawer holds secondary items only, never feature cards or browsable content.
4. Adaptive by default via window size classes (rememberWindowInfo), never hardcoded dp breakpoints: bottom bar on Compact, rail on Medium, permanent drawer on Expanded; constrain reading width and use two-pane (ListDetail input+result, paywall value+plans) on Medium/Expanded; support landscape, multi-window, and foldable postures.
...
- **Corrected:** Single NavigationSuiteScaffold exposing the peer destinations — bottom bar (Compact), rail (Medium), permanent drawer (Expanded) — with launchSingleTop + popUpTo(start){saveState} + restoreState. Drawer holds only secondary items. On Expanded, Text uses ListDetailPaneScaffold (input pane + result pane).
