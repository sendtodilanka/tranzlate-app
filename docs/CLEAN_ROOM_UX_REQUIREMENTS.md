...
| major | paywall | Segmented toggle used for plan comparison with weak hierarch | Use selectable billing-option cards per BUSINESS_MODEL.md §4 (Weekly/Monthly/Yearly, yearly pre-selected 'BEST VALUE'); each carries its own price + benefits — 'Recommende |
