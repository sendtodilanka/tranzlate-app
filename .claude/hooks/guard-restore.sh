...
#
# What is left here is only what a regex cannot answer and git can:
#
...
#
#   B. `git restore` carrying the literal `--staged` AND also asking for the
#      working tree (`--worktree` / `-W`). `man git-restore` OPTIONS: "If neither
#      option is specified, by default the working tree is restored… Specifying
#      both restores both." The hookify rule exempts `--staged` as a plain
#      SUBSTRING, so `--staged --worktree` slips its net; its own text says this
#      hook is the right place to catch it, because this hook can ask whether the
#      path is dirty. Short clusters (`-SW`, `-s@ -SW`) carry no literal
#      `--staged`, so hookify still blocks those and they are NOT handled here.
#
...
# A short flag cluster that includes W — `-W`, `-SW`, `-WS` — or `--worktree`.
has_worktree_flag() {
  local t
  for t in "$@"; do
    case "$t" in
...
while IFS= read -r seg; do
  ...
  while [ "$i" -lt "$n" ]; do
    case "${toks[$i]}" in
      -C)            workdir=${toks[$((i+1))]:-}; i=$((i+2)) ;;
      --work-tree)   workdir=${toks[$((i+1))]:-}; i=$((i+2)) ;;
      --work-tree=*) workdir=${toks[$i]#--work-tree=}; i=$((i+1)) ;;
  ...
  # `--` present → unambiguously a pathspec → hookify BLOCKS it. Not ours.
  ...
  if [ "$verb" = checkout ]; then
    ...
    # ARGUMENT DISAMBIGUATION: with one argument git prefers a tree-ish, and with
    # two the first IS the tree-ish. So drop candidate[0] when git can resolve it
    # as a commit — that command switches branches and loses nothing. Candidates
    # after the first are pathspecs regardless of what else they might name.
    if [ ${#cands[@]} -gt 0 ]; then
      probe=( git )
    ...
    # restore — ONLY the two-flag form that hookify's `--staged` substring exempts.
    staged=0
    for t in ${rest+"${rest[@]}"}; do [ "$t" = "--staged" ] && staged=1; done
    [ "$staged" = 1 ] || continue
    has_worktree_flag ${rest+"${rest[@]}"} || continue
    skip_next=0
    for t in ${rest+"${rest[@]}"}; do
      if [ "$skip_next" = 1 ]; then skip_next=0; continue; fi
      case "$t" in
