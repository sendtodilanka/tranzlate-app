...
**Why:** User මෙය mandatory project rule එකක් ලෙස add කරන්න කිව්වා (2026-07-21). Server-side branch protection එක free private repo (`sendtodilanka/Tranzlate`) එකට නැති නිසා (GitHub Pro ඕන), enforcement එන්නේ `.claude/hooks/guard-git.sh` (Claude `PreToolUse` hook, `guard-git.sh:46`) එකෙන්. **2026-08-01 (#170 co-verify):** මෙහි කලින් `.githooks/pre-push` සහ `.claude/hooks/block-push-to-main.sh` කියා ස්ථර දෙකක් නම් කර තිබුණි — **ඒ දෙකෙන් එකක්වත් නෑ** (`ls .githooks` → No such file). Doc එකක් code එකට ගළපා නොබැලීම — rule 11 හතරවන cause එකම.
