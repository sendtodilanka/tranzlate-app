...
class TranslateTextUseCaseTest {
    ...
    @Test
    fun `G11 metered ask at limit short-circuits to LimitReached without engine call`() = runTest {
    ...
    @Test
    fun `metered success increments usage once and asks ads`() = runTest {
    ...
    @Test
    fun `free engine success never charges the metered counter (C-10)`() = runTest {
        val usage = FakeUsagePolicy(left = 5)
        val ads = RecordingAdsCoordinator()
    ...
    @Test
    fun `failure increments nothing and never asks ads`() = runTest {
    ...
    @Test
    fun `AUTO at metered limit keeps working (C-11 - AUTO keeps working underneath)`() = runTest {
