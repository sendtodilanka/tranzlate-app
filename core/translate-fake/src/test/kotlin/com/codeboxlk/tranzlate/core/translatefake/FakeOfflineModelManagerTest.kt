...
class FakeOfflineModelManagerTest {
    ...
    @Test
    fun `seed covers all six states deterministically`() = runTest {
    ...
    @Test
    fun `download transitions NotDownloaded through Downloading to Downloaded`() = runTest {
    ...
    @Test
    fun `delete transitions Downloaded through Deleting to NotDownloaded`() = runTest {
        val manager = FakeOfflineModelManager()
        manager.modelStates().test {
    ...
    @Test
    fun `online-only rows have no download control`() = runTest {
        val manager = FakeOfflineModelManager()
    ...
    @Test
    fun `failed rows retry via download`() = runTest {
        val manager = FakeOfflineModelManager()
