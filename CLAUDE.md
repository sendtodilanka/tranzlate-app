...
**The gate is one command: `./gradlew preflight`** (#253). It runs the string-key catalogue check, detekt, spotless, **Android lint on every module's debug variants** (#315), every module's unit tests, **every androidTest source set's compilation**, and both debug APKs — the list lives in `build-logic/…/PreflightConventionPlugin.kt`, derived from the module and variant graph, so it cannot go stale in prose. The five-command line it replaces (`./gradlew test :app:assembleTranzlateProdDebug :app:assembleTranzlateFakeDebug spotlessCheck detekt`) compiled **no** androidTest source set at all: with a deliberate `Unresolved reference` in `app/src/androidTestProd/` it was BUILD SUCCESSFUL, exit 0, and `:core:database`'s androidTest was compiled by nothing whatsoever (#241). **What a green `preflight` does NOT mean:** instrumentation tests still do not RUN (#40, no emulator in CI), `build-logic`'s own Kotlin is still analysed by neither linter (#210), and Android lint runs on **debug variants only** — a release-only `lintVital` finding is still caught only by CI's `./gradlew build` (#315); and a screen's **design-conformance / screenshot lock** (a Roborazzi golden-diff) is verified by `build` ONLY — `preflight` runs the same unit tests but never the `verifyRoborazzi*` comparison, so a locked screen that has silently drifted from its golden stays GREEN under `preflight` and reddens only in CI (#337).
...
- **Enumerate before changing.** `grep -c` every call site and path of the thing being touched, and put **"Call sites: N found, N changed"** in the PR body. #146 converted 2 of 6; #150 fixed 5 of 9. Both were one grep away.
  - **`git grep -E` silently ignores `\b`** (#221): it returns 0 matches, exit 1 — *identical* to "appears nowhere" — so a count written `git grep -cE 'Sym\b'` is a fiction the `Call sites:` marker then presents as truth. Enumerate word-boundaried symbols with `grep -rE 'Sym\b' --include='*.kt' <dirs>`, `git grep -wE 'Sym'` (`-w` = whole word), or `git grep -E '(^|[^A-Za-z0-9_])Sym([^A-Za-z0-9_]|$)'`. **The signature that catches this class:** any `\b` enumeration that returns 0 while the bare term matches — re-run every `\b` count against the bare term. Enforced twice: `hookify.git-grep-word-boundary` blocks the command outright; `guard-pr.sh` warns when the idiom reaches a PR body (e.g. a `--body-file`) that the command-level block never sees.
...
**The plan is `docs/plan/issue-130-rev5-completion.md`** and it is `status: accepted`. It classifies all 59 then-open issues once, so the boundary is never re-argued per PR: **8 are new features he excluded and must NOT be built** (#7 #8 #20 #22 #78 #102 #139 #212 — they stay open and untouched), **10 are not rev5** — seven monetization/Access/paywall (including **#114, which means the shipped app earns nothing — deferred by scope, not by importance**), #191 History undo (owner-ruled, blocked on #241), #115 (fil/pt-BR native review, blocked on a human reviewer) and #116 (Composer/Result hygiene). The first draft called **nine** of them "monetization" (#191 always had its own clause); the #256 lens checked each and two are neither, and **41 are in scope**: 14 user-visible rev5 defects, 4 test gaps, 23 gates and process.
...
**What would retire this rule — stated so it is checkable, not felt:** every Phase 3 row in `docs/plan/issue-130-language-rev3.md` (PR-20…PR-28) shows ✅, **and** `docs/research/issue-130-rev5-audit.md` records all four passes with no design-invalidating finding. The first draft said "rev5 complete and #256 closed" — but #256 is a **PR**, so it closes on merge, months before rev5 is done, collapsing the condition to an undefined "rev5 complete". Caught by its own co-verify lens. **What would invalidate the plan rather than complete it:** if phase 2 returns findings that change the rev5 *design* rather than the code, phase 3's PR list is stale and the ruling reopens before anything is built — **#203 and #184 are the two most likely to do that**, both being frames that may not describe a producible state.
