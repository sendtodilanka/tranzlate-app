...
- **A search written from what I already know finds what I already know.** #171 missed a fifth call site because `CONTRIBUTING.md` said "Sinhala prose ≥70%" and my pattern was `70% Sinhala|Sinhala script`. #180 then asserted the cut was "recorded nowhere" — it was **two lines** below the passage I had just read. #183 attributed a mark to the wrong row from a **flattened** parse. → **Two independent searches that could not both miss the same thing**, named in the PR body. `guard-pr.sh` denies `gh pr create` without **`Enumerated by:`**.
...
- **A shared resource used without an exclusive claim.** Two agents on one emulator, twice (#187, #194). **Nothing errors** — `adb` serves both — so the symptom is a screenshot that disagrees with the code, which reads as a bug in the code. → `.claude/hooks/device-claim.sh` makes the contention visible. It **warns rather than denies**, because a hook sees a command and not a caller, and a guard that blocks the legitimate holder gets switched off.
- **A destructive command where a copy would do.** `git checkout --` destroyed uncommitted work twice; a conflict splice cut **mid-construct**, dropping a function's braces and the next KDoc's opener. → **`cp` for mutation backups. Never a boundary as a cut point.**
